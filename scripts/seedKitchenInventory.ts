/**
 * Kitchen Inventory Seeder
 * Reads kitchen_inventory_seed_part1.json & kitchen_inventory_seed_part2.json
 * and upserts all items into Firestore `inventory` collection.
 *
 * Run: npx tsx scripts/seedKitchenInventory.ts
 */
...
async function seedKitchenInventory() {
    console.log("🚀 Starting Kitchen Inventory Seeding...\n");
    const now = new Date();

    // Load both JSON files
    const part1Path = path.join(__dirname, "kitchen_inventory_seed_part1.json");
    const part2Path = path.join(__dirname, "kitchen_inventory_seed_part2.json");
    ...
    console.log(`\n🎉 Kitchen Inventory Seeding Complete!`);
...
seedKitchenInventory().catch(console.error);
