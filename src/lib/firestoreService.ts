...
const sampleAddOns: AddOn[] = [
  ...
  },
  {
    ...
    type: "per_room",
    description:
      "Create magical memories with a private candlelit dinner by the ocean. Enjoy a cozy beachfront setting, personalized service, and a complimentary bottle of sparkling wine to toast the evening under the stars.",
  ...
  },
  {
    ...
    type: "per_guest",
    description:
      "Embark on a breathtaking snorkeling adventure at Mnemba Atoll. Explore crystal-clear waters teeming with vibrant coral reefs and colorful marine life — a true underwater paradise.",
  ...
  },
  {
    ...
    type: "per_guest",
    description:
      "Unwind together with our signature couples' massage. Let expert therapists rejuvenate your body and mind in a serene setting inspired by the island's natural beauty — the perfect escape for two.",
  ...
  },
  {
    ...
    type: "per_room",
    description:
      "Travel in comfort with a private airport transfer designed for convenience and exclusivity. Each car accommodates up to four passengers, ensuring a smooth and private journey to and from the resort.",
...
const resolveRoomImage = (data: {
  ...
  if (name.includes("garden view") || name.includes("garden"))
    return roomImages.gardenSuite;
  if (name.includes("queen room") || name.includes("queen"))
    return roomImages.oceanSuite;
  if (
    name.includes("deluxe") ||
    name.includes("sea view") ||
...
export const getAllRoomTypes = async (): Promise<RoomTypeData[]> => {
  ...
  try {
    ...
    if (items.length === 0) {
      console.warn(
        " getAllRoomTypes: roomTypes empty, falling back to room_types",
      );
      const qLegacy = query(
        collection(db, "room_types"),
        orderBy("name", "asc"),
      ); // 'name' was often used in legacy
...
export const createAddOn = async (
  addOnData: Omit<AddOn, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) {
      ...
      if (guestDetails && (guestDetails.email || guestDetails.phone)) {
        ...
        if (!snapshot.empty) {
          ...
          await updateDoc(guestDoc.ref, {
            ...
            totalRevenue:
              (guestData.totalRevenue || 0) + (bookingData.totalAmount || 0),
  ...
  try {
    ...
    return querySnapshot.docs.map((doc) => {
      ...
      if (
        ...
        data.reservationGuests.length > 0
      ) {
...
export const syncGuestProfile = async (
  ...
  additionalData: { totalAmount?: number; isCheckIn?: boolean } = {},
) => {
  ...
  try {
    ...
    if (!snapshot.empty) {
      ...
      const updatePayload: any = {
        ...
        idDocumentNumber:
          guestDetails.idNumber || guestData.idDocumentNumber || "",
      };
      ...
      if (additionalData.totalAmount) {
        updatePayload.totalRevenue =
          (guestData.totalRevenue || 0) + additionalData.totalAmount;
      }
...
export const recalculateGuestStats = async (): Promise<{
  success: boolean;
  message: string;
}> => {
  ...
  try {
    ...
    for (const guest of allGuests) {
      ...
      const guestBookings = allBookings.filter((b) => {
        ...
        const emailMatch =
          gEmail && (bEmail === gEmail || flatEmail === gEmail);
        const phoneMatch =
          gPhone && (bPhone === gPhone || flatPhone === gPhone);
      ...
      if (
        ...
        Math.abs(totalRevenue - (guest.totalRevenue || 0)) > 1
      ) {
    ...
    return {
      success: true,
      message: `Successfully synced stats for ${updatedCount} guests.`,
    };
  } catch (error: any) {
...
export const createContactForm = async (
  contactData: Omit<ContactForm, "id" | "createdAt" | "updatedAt" | "status">,
): Promise<string | null> => {
  if (!db) {
...
export const createBookingEnquiry = async (
  data: Omit<BookingEnquiry, "id" | "createdAt" | "updatedAt" | "status">,
): Promise<string | null> => {
  if (!db) {
  ...
  try {
    console.log(
      "Creating booking enquiry in collection: bookingEnquiries",
      data,
    );
    ...
    console.log(
      "Booking enquiry created successfully with ID:",
      dr.id,
      "in collection: bookingEnquiries",
    );
    return dr.id;
  } catch (e) {
...
export const createExcursion = async (
  data: Omit<Excursion, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    const refCol = collection(db, "excursions");
...
export const addSystemLock = async (
  lockData: Omit<SystemLock, "id">,
): Promise<string> => {
...
export const createOffer = async (
  data: Omit<OfferBanner, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    const c = collection(db, "offers");
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Could not fetch discounts (collection may not exist):",
          getDocsError,
        );
        return [];
      }
...
export const createDiscountOffer = async (
  data: Omit<DiscountOffer, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    ...
    if (data.isActive) {
      try {
        ...
        console.warn(
          "Could not deactivate existing discounts:",
          deactivateError,
        );
      }
    }
...
export const updateDiscountOffer = async (
  id: string,
  data: Partial<Omit<DiscountOffer, "id" | "createdAt" | "updatedAt">>,
): Promise<boolean> => {
  if (!db) return false;
  try {
    // If setting this as active, deactivate all other discounts directly
    if (data.isActive === true) {
      try {
        const existingDiscounts = await getDiscountOffers();
        const activeDiscounts = existingDiscounts.filter(
          (d) => d.isActive && d.id !== id,
        );
        for (const discount of activeDiscounts) {
          const dref = doc(db, "discounts", discount.id);
          await updateDoc(dref, {
...
export const createSpecialOffer = async (
  data: Omit<
    SpecialOffer,
    "id" | "createdAt" | "updatedAt" | "lastNotificationSentAt"
  >,
): Promise<string | null> => {
  if (!db) return null;
  try {
...
export const updateSpecialOffer = async (
  id: string,
  data: Partial<Omit<SpecialOffer, "id" | "createdAt" | "updatedAt">>,
): Promise<boolean> => {
  if (!db) return false;
  try {
    const dref = doc(db, "specialOffers", id);
...
export const createStoryImage = async (
  data: Omit<StoryImage, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    const c = collection(db, "storyImages");
...
export const updateStoryImage = async (
  id: string,
  data: Partial<StoryImage>,
): Promise<boolean> => {
  if (!db) return false;
  try {
...
export const getGalleryImages = async (
  ...
  try {
    ...
    const qy = type
      ? query(c, orderBy("createdAt", "desc"))
      : query(c, orderBy("createdAt", "desc"));
    const snap = await getDocs(qy);
...
export const createRoomType = async (
  data: Omit<RoomType, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    const c = collection(db, "roomTypes");
...
export const createGuestExperienceForm = async (
  ...
  if (!db) {
    console.warn(
      "Firestore not available, cannot create guest experience form",
    );
    return null;
  }
...
export const getAllGuestExperienceForms = async (): Promise<
  GuestExperienceForm[]
> => {
  if (!db) {
...
export const getAllGuestReviews = async (
...
export const getMenuItems = async (
  ...
  if (!db) return [];
  try {
    const c = collection(db, "foodMenuItems");
    const qy = query(c, orderBy("createdAt", "desc"));
    const snap = await getDocs(qy);
    let items = snap.docs.map((d) => {
      const data = d.data();
      return {
        ...
        ...data,
        availableFrom:
          data.availableFrom?.toDate?.() || data.availableFrom || undefined,
        availableTo:
          data.availableTo?.toDate?.() || data.availableTo || undefined,
        createdAt: data.createdAt?.toDate() || new Date(),
        updatedAt: data.updatedAt?.toDate() || new Date(),
      } as MenuItem;
    });
...
export const getMenuItem = async (id: string): Promise<MenuItem | null> => {
  if (!db) return null;
  try {
    const r = doc(db, "foodMenuItems", id);
    const s = await getDoc(r);
    if (!s.exists()) return null;
    const data = s.data();
    return {
...
export const getMenuCategories = async (): Promise<MenuCategory[]> => {
  if (!db) return [];
  try {
    const c = collection(db, "foodCategories");
...
export const createMenuCategory = async (
  data: Omit<MenuCategory, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    const c = collection(db, "foodCategories");
...
export const updateMenuCategory = async (
  ...
  if (!db) return false;
  try {
    const r = doc(db, "foodCategories", id);
    const clean: any = {};
    Object.entries(data).forEach(([k, v]) => {
      if (v !== undefined) clean[k] = v;
    });
...
export const updateMenuItemsAvailability = async (
  ids: string[],
  isAvailable: boolean,
): Promise<boolean> => {
  if (!db) return false;
  try {
    await Promise.all(
      ids.map((id) => {
        ...
        const refDoc = doc(db, "foodMenuItems", id);
...
export const updateMenuItemsStatus = async (
  ids: string[],
  status: "draft" | "published" | "archived",
): Promise<boolean> => {
  if (!db) return false;
...
export const createGuestService = async (
  data: Omit<GuestService, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    // Remove undefined values - Firestore doesn't accept undefined
...
export const updateGuestService = async (
  id: string,
  data: Partial<GuestService> & { createTransaction?: boolean },
): Promise<boolean> => {
  if (!db) return false;
  try {
    ...
    if (createTransaction && updateData.status === "completed") {
      ...
      if (!service.bookingId || !chargeAmount || service.transactionId) {
        console.warn(
          "Skipping auto-transaction: Missing bookingId, amount, or already charged.",
        );
        return true;
      }
...
export const getLowStockInventory = async (): Promise<InventoryItem[]> => {
  ...
  try {
    ...
    return items.filter(
      (item) => item.currentStock <= (item.minStockLevel || 0),
    );
  } catch (e) {
...
export const getCheckoutBill = async (
  id: string,
...
export const generateCheckoutBill = async (
  bookingId: string,
): Promise<string | null> => {
  if (!db) return null;
  try {
    ...
    // Process Food Orders
    const foodOrders = foodOrdersSnap.docs.map(
      (d) => ({ id: d.id, ...d.data() }) as FoodOrder,
    );

    const validFoodOrders = foodOrders.filter(
      (o) => o.status !== "cancelled" && o.status !== "voided",
    );

    const foodCharges = validFoodOrders.reduce(
      (sum, order) => sum + (order.totalAmount || 0),
      0,
    );

    // Process Guest Services
    const services = guestServicesSnap.docs.map(
      (d) => ({ id: d.id, ...d.data() }) as GuestService,
    );

    const validServices = services.filter((s) => s.status !== "cancelled");

    const serviceCharges = validServices.reduce(
      (sum, service) => sum + (service.totalAmount || service.amount || 0),
      0,
    );

    // Process POS / Incidental Invoices
    const incidentalInvoicesSnap = await getDocs(
      query(
        collection(db, "incidentalInvoices"),
        where("bookingId", "==", bookingId),
      ),
    );

    const posInvoices = incidentalInvoicesSnap.docs.map(
      (d) => ({ id: d.id, ...d.data() }) as IncidentalInvoice,
    );

    // Filter valid POS invoices
    const validPosInvoices = posInvoices.filter((inv) => inv.status !== "void");

    const posCharges = validPosInvoices.reduce(
      (sum, inv) => sum + (inv.totalAmount || 0),
      0,
    );
    const posPayments = validPosInvoices.reduce(
      (sum, inv) => sum + (inv.totalPaid || 0),
      0,
    );

    // Add-ons charges
    const addOnsCharges = (booking.addOns || []).reduce(
    ...
    const serviceTransactionIds = new Set(
      validServices.map((s) => s.transactionId).filter(Boolean),
    );
    const foodTransactionIds = new Set(
      validFoodOrders.map((f) => (f as any).transactionId).filter(Boolean),
    );
    ...
    const uniqueTransactions = transactions.filter((t) => {
      ...
      if (
        t.reference?.startsWith("SVC-") &&
        validServices.some((s) =>
          t.reference?.includes(s.id.slice(-6).toUpperCase()),
        )
      )
        return false;
      if (
        t.reference?.startsWith("ORD-") &&
        validFoodOrders.some(
          (f) => f.orderNumber && t.reference?.includes(f.orderNumber),
        )
      )
        return false;
    ...
    const alreadyPaid =
      (booking.paidAmount || 0) + transactionPayments + posPayments;
    ...
    const roomNumber =
      booking.roomNumber ||
      (booking.rooms && booking.rooms[0]?.allocatedRoomType) ||
      null;
...
export const updateCheckoutBill = async (
  id: string,
  data: Partial<CheckoutBill>,
): Promise<boolean> => {
  if (!db) return false;
  try {
    const r = doc(db, "checkoutBills", id);
...
export const getRoomStatuses = async (
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "Index missing for roomStatuses, falling back to unsorted",
        e,
      );
      snap = await getDocs(c);
    }
...
export const createRoomStatus = async (
  data: Omit<RoomStatus, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    // Remove undefined values - Firestore doesn't accept undefined
...
export const completeRoomMaintenance = async (
  roomName: string,
): Promise<boolean> => {
  if (!db) return false;
  try {
    ...
    if (roomStatus.maintenanceStartDate && roomStatus.maintenanceEndDate) {
      const start =
        (roomStatus.maintenanceStartDate as any) instanceof Timestamp
          ? (roomStatus.maintenanceStartDate as any).toDate()
          : new Date(roomStatus.maintenanceStartDate);
      const end =
        (roomStatus.maintenanceEndDate as any) instanceof Timestamp
          ? (roomStatus.maintenanceEndDate as any).toDate()
          : new Date(roomStatus.maintenanceEndDate);
...
export const createHousekeepingTask = async (
  data: Omit<HousekeepingTask, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    // Remove undefined values - Firestore doesn't accept undefined
...
export const createCheckInOutRecord = async (
  data: Omit<CheckInOutRecord, "id" | "createdAt" | "updatedAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
    // Remove undefined values - Firestore doesn't accept undefined
...
export const checkInGuest = async (
  ...
  try {
    ...
    if (depositAmount !== undefined && depositAmount !== null)
      recordData.depositAmount = depositAmount;
...
export const checkOutGuest = async (
  ...
  try {
    ...
    const roomName =
      targetRoom.allocatedRoomType || booking.roomNumber || "Unknown";
    ...
    if (!checkInRecord) {
      console.warn(
        "No check-in record found for room, creating one...",
        roomName,
      );
      const suiteType = targetRoom.suiteType || "Garden Suite";
      ...
      if (checkInRecordId) {
        ...
        checkInRecord =
          records.find((r) => r.id === checkInRecordId) || records[0];
      }
    }
    ...
    const allCheckedOut = updatedRooms.every(
  ...
  try {
    ...
    let orders = querySnapshot.docs.map((doc) => {
      ...
      return {
        ...
        updatedAt: data?.updatedAt?.toDate() || new Date(),
        orderTime:
          data?.orderTime?.toDate() || data?.createdAt?.toDate() || new Date(),
      } as FoodOrder;
    });
...
// Void an order
export const voidFBOrder = async (
  orderId: string,
  reason: string,
): Promise<boolean> => {
  if (!db) return false;
...
// Change order owner
export const changeOrderOwner = async (
  orderId: string,
  ownerId: string,
): Promise<boolean> => {
  if (!db) return false;
...
// Split order
export const splitFBOrder = async (
  orderId: string,
  itemsToSplit: string[],
): Promise<string | null> => {
  if (!db) return null;
  try {
    ...
    const splitItems = originalItems.filter((item) =>
      itemsToSplit.includes(item.menuItemId),
    );
    const remainingItems = originalItems.filter(
      (item) => !itemsToSplit.includes(item.menuItemId),
    );
    ...
    const newOrderData = {
      ...
      totalAmount:
        splitSubtotal + (orderData.tax || 0) - (orderData.discount || 0),
    ...
    await updateDoc(orderRef, {
      ...
      totalAmount:
        remainingSubtotal + (orderData.tax || 0) - (orderData.discount || 0),
      updatedAt: serverTimestamp(),
    });
...
export const getFBRevenue = async (
  startDate: Date,
  endDate: Date,
): Promise<FBRevenue[]> => {
  if (!db) return [];
...
// Get Weekly Revenue
export const getFBWeeklyRevenue = async (
  year: number,
): Promise<FBWeeklyRevenue[]> => {
  if (!db) return [];
  try {
    ...
    const weeklyData: Record<
      number,
      { startDate: Date; endDate: Date; revenue: number; orders: number }
    > = {};
...
export const updateDailyFBRevenue = async (
  date: Date,
  orderData: FoodOrder,
): Promise<boolean> => {
  if (!db) return false;
  try {
    ...
    if (snapshot.empty) {
      ...
      await updateDoc(doc.ref, {
        ...
        totalDiscount:
          (existing.totalDiscount || 0) + (orderData.discount || 0),
        totalCustomers: (existing.totalCustomers || 0) + 1,
        totalVoid:
          existing.totalVoid +
          (orderData.status === "voided" ? orderData.totalAmount : 0),
        orderTypeSummary: {
          ...orderTypeSummary,
        ...
        paymentSummary: {
          ...paymentSummary,
          [paymentMethod]:
            (paymentSummary[paymentMethod] || 0) + orderData.totalAmount,
        },
        updatedAt: serverTimestamp(),
      });
    }
...
export const getMasterData = async <T>(
...
export const addMasterData = async <T>(
  collectionName: string,
  data: any,
  userInfo?: { user: string; ip?: string },
): Promise<string | null> => {
  if (!db) return null;
  try {
    const colRef = collection(db, collectionName);
...
export const checkMasterDataUsage = async (
  collectionName: string,
  id: string,
): Promise<UsageRecord[]> => {
  if (!db) return [];
...
export const addIncidentalInvoice = async (
  invoice: Omit<IncidentalInvoice, "id">,
) => {
  if (!db) throw new Error("Firestore unavailable");
  try {
...
export const updateIncidentalInvoice = async (
  id: string,
  data: Partial<IncidentalInvoice>,
  userInfo?: { user: string; ip?: string },
): Promise<boolean> => {
  if (!db) throw new Error("Firestore unavailable");
  try {
...
export const addFolioTransaction = async (
  transaction: Omit<FolioTransaction, "id" | "createdAt">,
) => {
  if (!db) return;
  try {
    ...
    if (transaction.type === "payment") {
      ...
      else if (method.includes("online") || method.includes("mpesa"))
        payMethod = "online";
...
export const getFolioTransactions = async (
  bookingId: string,
...
export const addTransaction = async (
  data: Omit<FolioTransaction, "id" | "createdAt">,
): Promise<string | null> => {
  if (!db) return null;
  try {
...
export const addGuest = async (
  guest: Omit<GuestProfile, "id" | "createdAt" | "updatedAt">,
) => {
