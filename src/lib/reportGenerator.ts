...
export const generateNightAuditPDF = (data: AuditReportData): Buffer => {
  ...
  doc.setFontSize(10);
  doc.text(
    `Business Date: ${data.businessDate.toLocaleDateString()}`,
    pageWidth - 14,
    25,
    { align: "right" },
  );
  doc.text(`Generated By: ${data.generatedBy}`, pageWidth - 14, 30, {
    align: "right",
  });
  ...
  const fbRev =
    data.fbData?.orders?.reduce((sum, o) => sum + (o.totalAmount || 0), 0) || 0;
  ...
  const ledgerData =
    data.financeData?.ledgerEntries.map((e) => [
  ...
  const fbOrders =
    data.fbData?.orders.map((o) => [
      o.orderNumber,
      o.roomName || o.deliveryLocation,
