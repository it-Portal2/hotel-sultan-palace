    ...
    >
      ...
      <header className="flex justify-between items-start border-b-2 border-black pb-6 mb-6">
        ...
        <div className="text-right">
          <h2 className="text-3xl font-light uppercase tracking-widest text-black/80 mb-4">
            Invoice
          </h2>
          <table className="text-right float-right">
            <tbody>
              <tr>
                <td className="pr-4 font-bold text-gray-600 uppercase text-xs">
                  Invoice #
                </td>
        ...
        <div className="w-1/2 pr-4 border-r border-gray-300">
          ...
          {bill.roomNumber && (
            <p className="text-gray-700">
              Room: <span className="font-medium">{bill.roomNumber}</span>
            </p>
          )}
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b-2 border-black">
              <th className="py-2 text-left text-xs font-bold uppercase tracking-wider w-1/12">
                #
              </th>
              <th className="py-2 text-left text-xs font-bold uppercase tracking-wider w-5/12">
                Description
              </th>
              <th className="py-2 text-center text-xs font-bold uppercase tracking-wider w-3/12">
                Date / Details
              </th>
              <th className="py-2 text-right text-xs font-bold uppercase tracking-wider w-3/12">
                Amount
              </th>
            </tr>
          </thead>
          <tbody className="text-sm">
            {/* 1. ROOM CHARGES */}
            {bill.roomDetails &&
              bill.roomDetails.length > 0 &&
              bill.roomDetails.map((room, i) => (
            ...
            {bill.services.map((svc, i) => (
              <tr key={`svc-${i}`} className="border-b border-gray-200">
                <td className="py-3 text-gray-500"></td>
                <td className="py-3">
                  <span className="font-bold block text-gray-900 capitalize">
      ...
      <footer className="mt-auto border-t border-gray-300 pt-6 text-center text-xs text-gray-500">
        ...
        <p>
          ...
          <strong>Sultan Palace Hotel</strong>.
        </p>
        <p>
          For inquiries, contact us at {hotelPhone} or {hotelEmail}
        </p>
      </footer>
