...
const getPortalFromPath = (path: string): PortalType => {
  ...
  if (
    ...
    return "kitchen";
  if (
    normalizedPath.startsWith("/admin/gallery") ||
    normalizedPath.startsWith("/admin/story-pictures") ||
    normalizedPath.startsWith("/admin/testimonials") ||
    normalizedPath.startsWith("/admin/offers") ||
    normalizedPath.startsWith("/admin/excursions") ||
    normalizedPath.startsWith("/admin/contacts") ||
    normalizedPath.startsWith("/admin/reputation-management") ||
    normalizedPath.startsWith("/admin/booking-enquiries")
  )
    return "website";
  if (normalizedPath.startsWith("/admin/accounts")) return "finance";
  if (
    normalizedPath.startsWith("/admin/staff") ||
    normalizedPath.startsWith("/admin/users") ||
    normalizedPath.startsWith("/admin/roles")
  )
    return "staff";
...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const filteredNavigationGroups = useMemo(() => {
    ...
    return groups.map((group) => ({
      ...
      items: group.items.map((item) => {
        ...
        const actualPortalKey =
          portal === "operations" ? "front_office" : portal;
  ...
  const isAuthorized = useMemo(() => {
    ...
    const allowList = (process.env.NEXT_PUBLIC_ADMIN_EMAILS || "")
      ...
      .filter(Boolean);
    const legacyAuth = userEmail
      ? allowList.includes(userEmail.toLowerCase())
      : false;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#FFFCF6] px-4">
        <div className="max-w-md w-full bg-white border border-[#be8c53]/20 p-6 text-center shadow-lg">
          <h1 className="text-2xl font-semibold text-[#202c3b]">
            Restricted Area
          </h1>
          <p className="mt-2 text-[#202c3b]/70">
            You are not authorized to access the admin panel.
          </p>
          <div className="mt-6 flex items-center justify-center gap-3">
            <Link
              href="/admin/login"
              className="inline-flex items-center bg-[#FF6A00] px-4 py-2 text-sm font-medium text-white hover:bg-[#FF6A00]/90 transition-colors"
            >
              Go to Login
            </Link>
            <Link
              href="/"
              className="inline-flex items-center bg-[#be8c53] px-4 py-2 text-sm font-medium text-white hover:bg-[#be8c53]/90 transition-colors"
            >
              Return to site
            </Link>
          </div>
        ...
        >
          <div className="flex h-16 items-center px-6 border-b border-gray-100 bg-white flex-shrink-0">
            <div className="flex items-center gap-3 w-full">
              ...
              <div
                className={`transition-opacity duration-300 ${sidebarOpen ? "opacity-100" : "opacity-0"}`}
              >
            ...
            {currentPortal === "selection" && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xl font-bold text-gray-900 tracking-tight">
                  Sultan Palace <span className="text-[#FF6A00]">Admin</span>
                </span>
              </div>
            )}
            {/* Quick Search - Center */}
              ...
              <div className="relative w-full">
                <button
                  onClick={() => {
                    ...
                    if (input?.value.trim()) {
                      router.push(
                        `/admin/bookings?query=${encodeURIComponent(input.value.trim())}`,
                      );
                    }
                  }}
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      ...
                      if (val) {
                        router.push(
                          `/admin/bookings?query=${encodeURIComponent(val)}`,
                        );
                      }
                    }
                  }}
                ...
                <div className="relative">
                  <div className="hidden sm:flex items-center gap-3">
                    <div className="relative">
                        ...
                        */}
                        {adminUser &&
                          adminUser.name !== "System Administrator" && (
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs font-semibold border rounded bg-gray-100 text-gray-700 border-gray-200`}
                            >
                      ...
                      {showUserMenu && (
                        <div
                          className="absolute right-0 mt-2 w-64 bg-white shadow-lg border border-gray-200 py-2 z-50"
                          onClick={(e) => e.stopPropagation()}
                        >
                          <div className="px-4 py-2 border-b border-gray-200">
                            <p className="text-sm font-semibold text-gray-900">
                              {adminUser?.name || userEmail.split("@")[0]}
                            </p>
                            ...
                            {adminUser &&
                              adminUser.name !== "System Administrator" && (
                                <span
                                  className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs font-semibold border mt-2 rounded bg-gray-100 text-gray-700 border-gray-200`}
                                >
                                  {roleLabels[adminUser.role] || adminUser.role}
                                </span>
                              )}
                  ...
                  <div className="sm:hidden">
                    <button
                      onClick={() =>
                        router.push(isFullAdmin ? "/admin/admin-users" : "#")
                      }
                      className="flex items-center gap-2 px-3 py-2 hover:bg-gray-50 transition-colors"
                    >
                      <UserCircleIcon className="h-5 w-5 text-gray-600" />
                      {isFullAdmin && (
                        <PlusIcon className="h-4 w-4 text-[#FF6A00]" />
                      )}
                    </button>
                  </div>
                </div>
            ...
            {isRestricted ? (
              <div className="flex flex-col items-center justify-center h-[60vh] text-center animate-fade-in">
                ...
                <p className="text-gray-500 mt-2 max-w-md">
                  You do not have permission to view this section. Please
