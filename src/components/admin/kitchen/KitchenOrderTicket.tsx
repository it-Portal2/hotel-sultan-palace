...
export default function KitchenOrderTicket({ order, onUpdateStatus }: KitchenOrderTicketProps) {
    ...
    const getUrgencyClasses = () => {
        if (order.status === 'delivered' || order.status === 'ready') return 'border-emerald-500/50 shadow-emerald-900/20';
        if (elapsedMinutes > 45) return 'border-red-500 shadow-red-900/50 animate-pulse ring-2 ring-red-500/20';
        if (elapsedMinutes > 30) return 'border-orange-500 shadow-orange-900/40 ring-1 ring-orange-500/20';
        return 'border-slate-600 shadow-lg';
    };

    const getTimerColor = () => {
        if (order.status === 'delivered' || order.status === 'ready') return 'text-emerald-400';
    ...
    return (
        <div className={`relative flex flex-col w-full bg-slate-700 rounded-lg overflow-hidden transition-all duration-300 border-l-4 ${getUrgencyClasses()}`}>
            ...
            <div className="p-3 bg-slate-800 border-b border-slate-600 flex justify-between items-start">
                <div>
                    ...
                    <h3 className="text-2xl font-black text-white leading-none font-mono tracking-tighter">
                        #{order.orderNumber.split('-')[1] || order.orderNumber.slice(-4)}
                    </h3>
                </div>
                <div className={`flex items-center gap-1.5 px-2 py-1 rounded bg-slate-900/50 border border-slate-600 ${getTimerColor()}`}>
            ...
            <div className="px-3 py-2 bg-slate-750 flex items-center justify-between border-b border-slate-600/50">
                <div className="flex items-center text-xs text-slate-300 font-bold bg-slate-600/50 px-2 py-1 rounded border border-slate-500">
                    ...
                    {order.roomNumber ? `ROOM ${order.roomNumber}` : (order.deliveryLocation?.toUpperCase() || 'N/A')}
                </div>
                <span className="text-[10px] font-mono text-slate-500">{new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
            </div>
            ...
            <div className="p-3 flex-grow bg-slate-700">
                <ul className="space-y-2">
                    {order.items.map((item, idx) => (
                        <li key={idx} className="text-sm pb-2 border-b border-slate-600 last:border-0 last:pb-0">
                            <div className="flex justify-between items-start font-mono">
                                <span className="font-bold text-white w-8 text-lg">{item.quantity}</span>
                                <span className="flex-1 text-slate-200 font-medium leading-tight pt-0.5">{item.name}</span>
                            </div>
