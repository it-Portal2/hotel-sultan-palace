...
export default function MenuTabContent({ menuType }: MenuTabContentProps) {
  ...
  // Service mapping based on menuType
  const services = useMemo(() => {
    return menuType === "bar"
      ? {
          getItems: getBarMenuItems,
          getCategories: getBarCategories,
          createCategory: createBarCategory,
          updateCategory: updateBarCategory,
          deleteCategory: deleteBarCategory,
          deleteItem: deleteBarMenuItem,
        }
      : {
          getItems: getFoodMenuItems,
          getCategories: getFoodCategories,
          createCategory: createFoodCategory,
          updateCategory: updateFoodCategory,
          deleteCategory: deleteFoodCategory,
          deleteItem: deleteFoodMenuItem,
        };
  }, [menuType]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.error(`Error fetching ${menuType} menu data:`, error);
      showToast(`Failed to load ${menuType} menu data`, "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await services.deleteItem(deleteConfirm);
  ...
  const handleCategorySubmit = async (data: Partial<FoodCategory>) => {
    ...
    try {
      if (editingCategory) {
        await services.updateCategory(editingCategory.id, data);
        showToast("Category updated successfully", "success");
      } else {
        await services.createCategory(data);
  ...
  const executeDeleteCategory = async () => {
    ...
    try {
      await services.deleteCategory(deleteCatConfirm.id);
      fetchData();
  ...
  // Filter out items that don't belong to any loaded category (e.g. Bar items in Food tab)
  ...
  }, [validItems, searchTerm, selectedCategoryId, categories]);
  ...
  return (
    <div className="flex h-full bg-gray-50 overflow-hidden relative">
      ...
      {/* Sidebar */}
      <aside
        className={`absolute inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 flex flex-col transition-transform duration-300 md:translate-x-0 ${
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
        } md:static md:flex h-full shadow-xl md:shadow-none flex-shrink-0`}
      >
      ...
      <main className="flex-1 flex flex-col h-full min-w-0 overflow-hidden relative">
        <header className="bg-white border-b border-gray-200 px-4 md:px-6 py-4 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between flex-shrink-0 z-10 w-full">
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-gray-800 tracking-tight flex items-center gap-3 capitalize">
                {menuType} Menu
              </h1>
              <p className="text-gray-500 text-xs md:text-sm font-medium mt-1">
                Manage your {menuType} offerings
              </p>
            </div>
          </div>
