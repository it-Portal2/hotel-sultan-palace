...
export default function FolioDetailsDrawer({
  booking,
  open,
  onClose,
}: FolioDetailsDrawerProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const validPosInvoices = allInvoices.filter(
        (inv) => inv.bookingId === booking.id && inv.status !== "void",
      );
  ...
  const roomTotal =
    booking.rooms?.reduce((acc, r) => acc + (r.price || 0), 0) * nights;

  // 1b. Meal Plan Charges
  const mealPlanTotal =
    booking.rooms?.reduce((acc, r) => acc + (r.mealPlanPrice || 0), 0) * nights;
  ...
  // 3. Service Charges
  const serviceTotal = guestServices.reduce(
    (acc, s) => acc + (s.totalAmount || s.amount || 0),
    0,
  );

  // 3b. POS Charges
  const posTotal = posInvoices.reduce(
    (acc, inv) => acc + (inv.totalAmount || 0),
    0,
  );
  // Note: If POS was paid by cash, we need to credit it below.
  const posPayments = posInvoices.reduce(
    (acc, inv) => acc + (inv.totalPaid || 0),
    0,
  );
  ...
  const serviceTransactionIds = new Set(
    guestServices.map((s) => s.transactionId).filter(Boolean),
  );
  const foodTransactionIds = new Set(
    foodOrders.map((f) => (f as any).transactionId).filter(Boolean),
  ); // If food orders have transaction link
  ...
  const otherCharges = uniqueTransactions
    .filter((t) => t.type === "charge")
    .reduce((acc, t) => acc + (t.amount || 0), 0);
  const totalPayments = uniqueTransactions
    .filter((t) => t.type === "payment")
    .reduce((acc, t) => acc + (t.amount || 0), 0);

  const unaccountedPaidAmount = Math.max(
    0,
    (booking.paidAmount || 0) - totalPayments,
  );
                          ...
                          <tbody className="divide-y divide-gray-100">
                            ...
                            <tr>
                              <td className="px-6 py-4 font-medium text-gray-900">
                                Room Charge (
                                {booking.rooms?.[0]?.allocatedRoomType}) x{" "}
                                {nights} Night(s)
                              </td>
                            ...
                            {mealPlanTotal > 0 && (
                              <tr>
                                <td className="px-6 py-4 font-medium text-gray-900">
                                  Meal Plan Charges (
                                  {booking.rooms?.[0]?.mealPlan || "Meal Plan"})
                                  x {nights} Night(s)
                                </td>
                                <td className="px-6 py-4 text-right text-gray-900 font-mono">
                                  {mealPlanTotal.toLocaleString()}
                                </td>
                                <td className="px-6 py-4 text-right text-gray-400 font-mono">
                                  -
                                </td>
                              ...
                              <tr className="bg-green-50/30">
                                <td className="px-6 py-4 font-medium text-green-700">
                                  {booking.discount?.code
                                    ? `Discount Applied (${booking.discount.code})`
                                    : "System Adjustment / Discount"}
                                </td>
                                <td className="px-6 py-4 text-right text-gray-400 font-mono">
                                  -
                                </td>
                                <td className="px-6 py-4 text-right text-green-700 font-bold font-mono">
                              ...
                              <React.Fragment key={inv.id}>
                                ...
                                {inv.totalPaid > 0 && (
                                  <tr className="bg-purple-50/20">
                                    ...
                                    <td className="px-6 py-4 text-right text-purple-700 font-mono">
                            ...
                            {uniqueTransactions.map((t) => (
                              <tr
                                key={t.id}
                                className={
                                  t.type === "payment" ? "bg-emerald-50/30" : ""
                                }
                              >
                                <td className="px-6 py-4 text-gray-900">
                                  <span className="block font-medium">
                                ...
                                {t.type === "payment" ? (
                                  <>
                                    ...
                                    <td className="px-6 py-4 text-right text-emerald-700 font-bold font-mono">
                            ...
                            {unaccountedPaidAmount > 0 && (
                              <tr className="bg-emerald-50/30">
                                <td className="px-6 py-4 font-medium text-emerald-800">
                                  Advance Deposit / Initial Payment
                                </td>
                                <td className="px-6 py-4 text-right text-gray-400 font-mono">
                                  -
                                </td>
                                <td className="px-6 py-4 text-right text-emerald-700 font-bold font-mono">
                                  {unaccountedPaidAmount.toLocaleString()}
                                </td>
                              </tr>
                            )}
                            ...
                            <tr className="bg-gray-50 font-bold border-t-2 border-gray-100">
                              <td className="px-6 py-4 text-right uppercase text-xs tracking-wide">
                                Total Billed
                              </td>
                            ...
                            <tr className="bg-gray-100 font-bold border-t-2 border-gray-200">
                              ...
                              <td
                                className="px-6 py-4 text-right text-red-600 text-xl"
                                colSpan={2}
                              >
