...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!token) {
             return NextResponse.json({ Result: '001', ResultExplanation: 'Transaction token is required' }, { status: 400 });
        }
        ...
        if (!companyToken || !apiEndpoint) {
            return NextResponse.json(
                { Result: '001', ResultExplanation: 'DPO Setup Error: Credentials missing for verification.' },
                { status: 500 }
            );
        }
