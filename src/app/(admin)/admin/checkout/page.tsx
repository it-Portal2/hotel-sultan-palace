...
import CheckOutModal, {
  CheckOutData,
} from "@/components/admin/front-desk/CheckOutModal";
import {
  MagnifyingGlassIcon,
  CreditCardIcon,
  CheckCircleIcon,
...
export default function AdminCheckoutPage() {
  ...
  const handleCheckOut = async (data: CheckOutData) => {
    ...
    try {
      ...
      const roomNum =
        ...
        selectedBooking.rooms[0]?.allocatedRoomType;
      const activeLock = locks.find(
        (lock) =>
          (lock.resourceType === "folio" &&
            lock.resourceId === selectedBooking.id) ||
          (roomNum &&
            lock.resourceType === "room" &&
            lock.resourceId === roomNum),
      );

      if (activeLock) {
        showToast(
          `Checkout Blocked: Folio is locked by ${activeLock.lockedBy}. Reason: ${activeLock.description}`,
          "error",
        );
        setProcessing(false);
        return;
    ...
    try {
      const success = await checkOutGuest(selectedBooking.id, {
  ...
  const displayedBookings = useMemo(() => {
    ...
    return bookingsToShow.filter((booking) => {
      ...
      const guestName =
        `${booking.guestDetails?.firstName || ""} ${booking.guestDetails?.lastName || ""}`.toLowerCase(); // Adapted to existing guestDetails structure
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Process checkout and manage final bills • {currentDate}
          </p>
        </div>
      ...
      {displayedBookings.length === 0 ? (
        <div className="text-center py-16 bg-white border border-gray-200 shadow-sm">
          ...
          <p className="text-sm text-gray-500 mt-2">
            {viewMode === "ready"
              ? "All checked-in guests will appear here"
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {displayedBookings.map((booking) => {
                  ...
                  return (
                    <tr
                      key={booking.id}
                      className={`hover:bg-gray-50 transition-colors ${isCheckedOut ? "bg-green-50/10" : ""}`}
                    >
                          ...
                          {!isCheckedOut &&
                            (isReadOnly ? (
                              ...
                              <button
                                ...
                                title={
                                  !hasBill ? "Please generate bill first" : ""
                                }
                              >
                                <UserMinusIcon className="h-4 w-4" />
                                Check Out
          ...
          >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6 bg-gray-50 p-4 border border-gray-100">
                <div>
                  ...
                  <p className="text-sm text-gray-700">
                    {checkoutBill.guestName}
                  </p>
                  {checkoutBill.roomNumber && (
                    <p className="text-sm text-gray-700 mt-1">
                      Room: {checkoutBill.roomNumber}
                    </p>
                  )}
                </div>
                <div>
                  ...
                  <p className="text-sm text-gray-700">
                    Check-in:{" "}
                    {new Date(checkoutBill.checkInDate).toLocaleDateString()}
                  </p>
                  <p className="text-sm text-gray-700">
                    Check-out:{" "}
                    {new Date(checkoutBill.checkOutDate).toLocaleDateString()}
                  </p>
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                <div className="mb-4 border-b border-gray-100 pb-3">
                  <div className="flex justify-between items-center py-2">
                    ...
                    <span className="text-sm font-bold text-gray-900">
                      ${checkoutBill.roomCharges.toFixed(2)}
                    </span>
                  </div>
                  {checkoutBill.roomDetails.map((room, idx) => (
                    <div
                      key={idx}
                      className="flex justify-between items-center py-1 text-xs text-gray-600 pl-4 mt-1"
                    >
                      <span>
                        {room.roomType} ({room.nights} nights × ${room.rate}
                        /night)
                      </span>
                ...
                {checkoutBill.foodCharges > 0 && (
                  <div className="mb-4 border-b border-gray-100 pb-3">
                    <div className="flex justify-between items-center py-2">
                      ...
                      <span className="text-sm font-bold text-gray-900">
                        ${checkoutBill.foodCharges.toFixed(2)}
                      </span>
                    </div>
                ...
                {checkoutBill.serviceCharges > 0 && (
                  <div className="mb-4 border-b border-gray-100 pb-3">
                    <div className="flex justify-between items-center py-2">
                      ...
                      <span className="text-sm font-bold text-gray-900">
                        ${checkoutBill.serviceCharges.toFixed(2)}
                      </span>
                    </div>
                ...
                {checkoutBill.customCharges > 0 && (
                  <div className="mb-4 border-b border-gray-100 pb-3">
                    <div className="flex justify-between items-center py-2">
                      <span className="text-sm font-semibold text-gray-700">
                        Custom Charges & Transactions
                      </span>
                      <span className="text-sm font-bold text-gray-900">
                        ${checkoutBill.customCharges.toFixed(2)}
                      </span>
                    </div>
                    ...
                    {(checkoutBill as any).transactions
                ...
                {checkoutBill.addOnsCharges > 0 && (
                  <div className="mb-4 border-b border-gray-100 pb-3">
                    <div className="flex justify-between items-center py-2">
                      ...
                      <span className="text-sm font-bold text-gray-900">
                        ${checkoutBill.addOnsCharges.toFixed(2)}
                      </span>
                    </div>
                ...
                {checkoutBill.discount && checkoutBill.discount > 0 && (
                  <div className="flex justify-between items-center py-2 border-b border-gray-100">
                    ...
                    <span className="text-sm font-semibold text-green-600">
                      -${checkoutBill.discount.toFixed(2)}
                    </span>
                  </div>
                )}
                ...
                <div className="flex justify-between items-center py-4 border-t-2 border-gray-300 mt-4">
                  ...
                  <span className="text-3xl font-bold text-[#FF6A00]">
                    ${checkoutBill.totalAmount.toFixed(2)}
                  </span>
                </div>
                ...
                <div className="mt-4 pt-4 border-t-2 border-gray-200 bg-gray-50 p-4 border border-gray-100">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-sm font-semibold text-gray-900">
                      ${checkoutBill.paidAmount.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                    ...
                    <span
                      className={`text-xl font-bold ${checkoutBill.balance > 0 ? "text-red-600" : "text-green-600"}`}
                    >
                  ...
                  <div className="mt-3">
                    <span
                      className={`inline-flex items-center px-3 py-1.5 text-xs font-semibold uppercase ${
              ...
              <div className="text-center text-xs text-gray-500 border-t border-gray-200 pt-4">
                ...
                <p className="mt-1">
                  Bill Generated:{" "}
                  {new Date(checkoutBill.createdAt).toLocaleString()}
                </p>
              </div>
            </div>
          </div>
