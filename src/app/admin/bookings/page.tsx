...
export default function AdminBookingsPage() {
  ...
  const [confirmAction, setConfirmAction] = useState<{ type: 'cancel' | 'confirm' | 'pending', booking: Booking } | null>(null);
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex gap-1 border-b-2 border-gray-200 pb-2">
            <button
              ...
              className={`px-3 py-1 text-sm font-medium transition-colors ${status === 'all'
            ...
            <button
              ...
              className={`px-3 py-1 text-sm font-medium transition-colors ${status === 'pending'
                ? 'text-[#FF6A00] border-b-2 border-[#FF6A00] -mb-[2px]'
                : 'text-gray-600 hover:text-gray-900'
                }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 text-sm font-medium transition-colors ${status === 'confirmed'
                ? 'text-[#FF6A00] border-b-2 border-[#FF6A00] -mb-[2px]'
                : 'text-gray-600 hover:text-gray-900'
                }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 text-sm font-medium transition-colors ${status === 'cancelled'
                ? 'text-[#FF6A00] border-b-2 border-[#FF6A00] -mb-[2px]'
                : 'text-gray-600 hover:text-gray-900'
                }`}
            >
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-gray-50 border-t border-gray-200">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${page <= 1
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${page >= totalPages
                  ? 'text-gray-400 border-gray-200 cursor-not-allowed'
                  : 'text-gray-700 border-[#FF6A00] hover:text-[#FF6A00]'
                  }`}
              >
                  ...
                  <div className="mt-6 bg-gray-50 rounded-lg p-6 border border-gray-200">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="text-right">
                        ...
                        <span className={`inline-block px-4 py-2 rounded-full text-sm font-semibold ${selected.status === 'confirmed' ? 'bg-green-100 text-green-800' :
                      ...
                      {isReadOnly ? (
                        ...
                        <>
                          {selected.status !== 'confirmed' && (
                            <button
                              onClick={() => setConfirmAction({ type: 'confirm', booking: selected })}
                          ...
                          {selected.status === 'confirmed' && (
                            <button
                              onClick={() => setConfirmAction({ type: 'pending', booking: selected })}
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 animate-in fade-in zoom-in duration-200">
              ...
              <p className="text-gray-600 mb-6">
                {confirmAction.type === 'cancel'
                  ? 'Are you sure you want to cancel this booking? The room will be freed up automatically. This action cannot be undone.'
                  : confirmAction.type === 'confirm'
                    ? 'This will mark the booking as confirmed and notify the guest.'
                    : 'This will revert the booking status to pending.'}
              </p>
              <div className="flex gap-3 justify-end">
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      if (action === 'cancel') {
                        ...
                        if (selected?.id === booking.id) setSelected({ ...selected, status: 'cancelled' });
                      } else if (action === 'confirm') {
                        await confirmBooking(booking.id);
                        showToast('Booking confirmed successfully', 'success');
                        if (selected?.id === booking.id) setSelected({ ...selected, status: 'confirmed' });
                      } else if (action === 'pending') {
                        await updateBooking(booking.id, { status: 'pending' });
                        showToast('Booking marked as pending', 'success');
                        if (selected?.id === booking.id) setSelected({ ...selected, status: 'pending' });
                      }
                  ...
                  className={`px-4 py-2 text-white rounded-lg font-medium transition-colors ${confirmAction.type === 'cancel' ? 'bg-red-600 hover:bg-red-700' :
                    'bg-blue-600 hover:bg-blue-700'
                    }`}
                >
                  {confirmAction.type === 'cancel' ? 'Yes, Cancel Booking' : 'Yes, Confirm'}
                </button>
              </div>
            </div>
          </div>
